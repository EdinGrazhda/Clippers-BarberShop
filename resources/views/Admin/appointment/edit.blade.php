                ...
                <div class="flex items-center justify-between">
                    <div>
                        <div class="flex items-center mb-2">
                            <div class="p-3 bg-gradient-to-br from-amber-500 to-orange-600 rounded-xl mr-4">
                                <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                                </svg>
                            </div>
                            <h1 class="text-4xl font-bold bg-gradient-to-r from-gray-900 to-gray-600 dark:from-gray-100 dark:to-gray-300 bg-clip-text text-transparent">Edit Appointment</h1>
                        </div>
                        <p class="text-gray-600 dark:text-gray-400 text-lg">Update appointment details for {{ $appointment->customer_name }}</p>
                    </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700 p-8">
                <form action="{{ route('appointments.update', $appointment) }}" method="POST">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input type="datetime-local" name="appointment_time" id="appointment_time" 
                                   value="{{ old('appointment_time', $appointment->appointment_time?->format('Y-m-d\TH:i')) }}" required 
                                   class="w-full px-4 py-3 bg-white dark:bg-gray-900 border border-gray-300 dark:border-gray-700 text-gray-900 dark:text-gray-100 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500">
                        </div>
                            ...
                            <select name="status" id="status" required 
                                ...
                                @foreach($appointmentStatuses as $status)
                                    <option value="{{ $status->value }}" 
                                            {{ old('status', $appointment->status->value) == $status->value ? 'selected' : '' }}>
                                        {{ $status->label() }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea name="notes" id="notes" rows="4" maxlength="1000" 
                                      ...
                                      class="w-full px-4 py-3 bg-white dark:bg-gray-900 border border-gray-300 dark:border-gray-700 text-gray-900 dark:text-gray-100 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500">{{ old('notes', $appointment->notes) }}</textarea>
                        </div>
                    </div>
                    ...
                    <div class="mt-8 flex flex-col sm:flex-row sm:justify-end space-y-4 sm:space-y-0 sm:space-x-4">
                        ...
                        <button type="submit" 
                                class="w-full sm:w-auto px-6 py-3 bg-gradient-to-r from-amber-600 to-orange-600 hover:from-amber-700 hover:to-orange-700 text-white font-medium rounded-lg transition-colors duration-200">
                            Update Appointment
                        </button>
                    </div>
                </form>
